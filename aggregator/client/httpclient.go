...
func (c *HTTPClient) Aggregate(ctx context.Context, aggReq *types.AggregateRequest) error {
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("the service responded with no 200 status code %d", resp.StatusCode)
	}
	resp.Body.Close()
	return nil
}
...
func (c *HTTPClient) GetInvoice(ctx context.Context, id int) (*types.Invoice, error) {
	...
	slog.Info(fmt.Sprintf("requesting get invoice -> %s", endpoint))
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("the service responded with no 200 status code %d", resp.StatusCode)
	}
